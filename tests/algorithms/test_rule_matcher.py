...
def test_find_violations():
    code = ["import os", "password = input()", "print(password)"]

    rules = [{"pattern": "password = input()", "severity": "HIGH"}]
...
def test_find_violations_v2():
    ...
    rules = [{"pattern": "eval($X)", "severity": "HIGH"}]
    ...
    expected_result = [
        {
            ...
            "pattern": "eval($X)",
            "severity": "HIGH",
            "captures": {"X": "user_input"},
        },
        {
            "line_number": 4,
            "pattern": "eval($X)",
            "severity": "HIGH",
            "captures": {"X": "request.args['cmd']"},
        },
    ]
    print("RESULT from find violations v2:", find_violations_v2(code, rules))
